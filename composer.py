...
def play():
    ...
    while running:
            ...
            if event.type == pygame.QUIT:  # QUIT BUTTON HIT
                ...
                if event.key == pygame.K_o:  # KEYDOWN O
                    ...
                    if is_ae:
                        ...
                        #latent_x = encoder.predict(np.expand_dims(example_song, 0), batch_size=1)[0].numpy()
                        latent_x = encoder(np.expand_dims(example_song, 0)).numpy()
                        cur_len += y_lengths[random_song_ix]
                        random_song_ix += 1
                    else:
                        print("NO AE")
                        random_song_ix = np.array([random_song_ix], dtype=np.int64)
                        #latent_x = encoder.predict(random_song_ix, batch_size=1)[0].numpy()
                        latent_x = encoder(random_song_ix).numpy()
                ...
                if event.key == pygame.K_c:  # KEYDOWN C
                    ...
                    #latent_x = encoder.predict(y)[0].numpy()
                    latent_x = encoder(y).numpy()
                    if use_pca:
                        current_params = np.dot(latent_x - latent_means, latent_pca_vectors.T) / latent_pca_values
                    else:
                        current_params = (latent_x - latent_means) / latent_stds
        ...
        if needs_update:
            ...
            y = decoder(latent_x).numpy()[0]
            current_notes = np.array((y * 255.0))
            needs_update = False
